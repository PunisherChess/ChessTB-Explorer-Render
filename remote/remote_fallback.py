...
``_TableFile._open_source()`` resolves a real filesystem path with
...
``_open_source`` is itself a documented seam another transport can
replace -- ``remote_direct.py`` does exactly that, subclassing it to read
HTTP byte ranges on demand. This module takes the simpler of the two
approaches: it leaves ``_open_source`` untouched and overrides only
``_find``, described below.
...
rather than
a local ``os.path.exists()`` check). That's a performance-only characteristic,
not a correctness gap, and may remain installed -- see remote_direct.py's
module docstring for the alternative (per-``(kind, material)`` locking) it
uses instead.
"""
...
class _RemoteDiskCache:
    ...
    each -- whole files here instead of fixed-size pages, since
    ...
    the *front* -- so the file ``_find()`` resolved a path for can
    ...
    it is closed, so an in-flight ``open().read()`` (the only operation the
    ...
    def __init__(self, root: str, max_bytes: int) -> None:
        ...
        # that tail. On POSIX, rmtree-ing a directory containing a
        ...
        # FileNotFoundError for a missing source path
    ...
    def has(self, rel_path: str) -> bool:
        ...
        thread cannot acquire it (see class docstring)."""
        with self._lock:
    ...
    def record(self, rel_path: str, size: int) -> None:
        with self._lock:
            ...
            # Keep the newly added file (it is at the end); never evict fully
...
class _RemoteCachingTablebase(chesstb.Tablebase):
    ...
    def __init__(self, base_url: str, *, block_cache_bytes: int,
        ...
        # Base Tablebase.__init__ calls self.add_directory(base_url) --
        # overridden to a no-op below, since _find here resolves directly
        # against "<kind>/" and never falls back to a bare-root candidate
        # the way local _find does -- the published remote layout always
        # has the kind subdirectories, per README.md's "Getting the
        # tablebase files".
        super().__init__(base_url, block_cache_bytes=block_cache_bytes)

    def add_directory(self, base_dir: str) -> None:
        """No-op -- see the comment in __init__ above."""
    ...
    def _ensure_cached(self, rel_path: str) -> Optional[str]:
        ...
        with self._disk_cache.lock_for(rel_path):
            ...
            # _active_downloads for why racing it (rather than
    ...
    def clear_caches(self) -> None:
        ...
        Tablebase would re-download every table on first touch)."""
...
def open_tablebase(directory: str, *,
    ...
    ``chess.chesstb.open_tablebase`` for a local path, so app.py can select
