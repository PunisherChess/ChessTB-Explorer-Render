...
* :class:`RemoteFileView` -- a lightweight ``(RemoteFile, base, length)``
  ...
  never touches the network; it narrows the (base, length) window.
...
#: this against ``PROBE_THREADS``) does not specify one explicitly. One
...
class _PageCacheShard:
    ...
    def __init__(self, max_bytes: int) -> None:
        ...
        self._page_locks: weakref.WeakValueDictionary = weakref.WeakValueDictionary()
        self._meta_lock = threading.Lock()

    def lock_for(self, key: Tuple[str, int]) -> threading.Lock:
        # The caller holds a strong reference while waiting/reading. A
        # weak registry retains the same lock for every concurrent caller
        # but releases it once nobody uses it, independently of eviction.
    ...
    def put(self, key: Tuple[str, int], data: bytes) -> None:
        with self._lock:
            ...
            while self.cur_bytes > self.max_bytes and len(self._lru) > 1:
                ...
                # Lock lifetime follows active users, not cached bytes.
    ...
    def clear(self) -> None:
        with self._lock:
            ...
            self.hits = 0
            self.misses = 0
        # Do not reset lock identities: an in-flight fetch (and its
        # waiters) must still serialize with a read arriving after clear.
        # Weak entries disappear automatically when the last user exits.
...
class _PageCache:
    ...
    def __init__(self, max_bytes: int) -> None:
        self._shard_count = DEFAULT_PAGE_CACHE_SHARDS
...
class RemoteHTTPClient:
    ...
    def __init__(self, base_url: str, *, timeout: float = DEFAULT_TIMEOUT,
                 ...
                 pool_maxsize: int = DEFAULT_POOL_MAXSIZE) -> None:
        if requests is None:
            raise RuntimeError(
    ...
    def get_range(self, path: str, offset: int, length: int) -> bytes:
        ...
        for attempt in range(self.max_retries):
            ...
            if resp.status_code == 200:
                # Range not honoured by this server/CDN -- stream=True
                # above means the body isn't downloaded yet at this point,
                # so a declared (or entirely undeclared -- chunked
                # transfer-encoding, or a server that omits
                # Content-Length) size can still be checked before
                # `.content` below pulls it fully into memory. A
                # compliant 206 always carries Content-Range and a body
                # sized to the request; a 200 that comes back this large
                # (or of unknown size) means the *whole* object came back,
                # which for a multi-GB tablebase file is not something to
                # buffer to slice a few hundred KiB out of it.
                cl = resp.headers.get("Content-Length")
                declared_size = int(cl) if cl is not None and cl.isdigit() else None
                if declared_size is None or declared_size > length:
                    resp.close()
                    raise RemoteSourceError(
                        f"{url} ignored the Range request and returned status 200 "
                        f"with {'an unknown-size' if declared_size is None else declared_size} "
                        f"body (requested {length} bytes) -- refusing to buffer "
                        f"the full object in memory."
                    )
                self._record(len(resp.content))
            ...
            self._record(len(resp.content) if resp.content is not None else 0)
            last_exc = RemoteSourceError(
    ...
    def get_first_page(self, path: str, page_size: int) -> Tuple[Optional[int], bytes]:
        ...
        This combines existence, size, and header retrieval in one round
        trip: a freshly touched table needs all three values, and the
        initial page is already required for header parsing. The returned
        data is reused for the first page of the remote file. `data` is capped to `page_size` bytes
        even if the server ignores the Range header and returns a whole
        object no bigger than `page_size` -- a genuinely small file, the
        one case worth still buffering. A larger (or size-unknown, e.g.
        chunked) object coming back whole instead raises
        :class:`RemoteSourceError` rather than buffering it in full; see
        the ``status_code == 200`` branch below.
        """
        if page_size <= 0:
            return None, b""
        url = self.url_for(path)
        ...
        for attempt in range(self.max_retries):
            try:
                resp = self._session.get(
                    url, headers={"Range": f"bytes=0-{page_size - 1}"},
                    timeout=self.timeout, stream=True,
                )
            except requests.RequestException as exc:
                self._record()
            ...
            if resp.status_code == 200:
                # Range not honoured -- could be a genuinely tiny file
                # (smaller than page_size, the legitimate case the
                # docstring above describes) or a large-object-ignoring-
                # Range CDN returning the whole thing from byte 0. As in
                # get_range(), stream=True above means Content-Length can
                # be checked before `.content` reads the body, so only the
                # first, legitimate case actually gets buffered here.
                cl = resp.headers.get("Content-Length")
                declared_size = int(cl) if cl is not None and cl.isdigit() else None
                if declared_size is None or declared_size > page_size:
                    resp.close()
                    raise RemoteSourceError(
                        f"{url} ignored the Range request and returned status 200 "
                        f"with {'an unknown-size' if declared_size is None else declared_size} "
                        f"body (page_size is {page_size} bytes) -- refusing to buffer "
                        f"the full object in memory."
                    )
                self._record(len(resp.content))
    ...
    def read(self, offset: int, length: int) -> bytes:
        ...
        for page_index in range(first_page, last_page + 1):
            ...
            if len(page) < expected_page_len:
                ...
                # length -- not only take == 0 above, which catches
...
class RemoteFileView:
    ...
    def get_bytes(self) -> bytes:
        """Materialise (fetch, via the page cache) this whole view's
        bytes. Slice first (``view[a:b]``) to materialise a sub-range."""
        return self._file.read(self._base, self._len)
