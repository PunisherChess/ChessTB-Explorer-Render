...
contains the primary application configuration. A subset of settings is
also readable from the process environment, with the literal value below
used when the environment variable is unset, allowing one committed
configuration file to serve multiple deployment platforms.
  ...
  - PORT — read from the environment with a literal fallback, because
    ...
    (via the platform's PORT variable) rather than using a fixed value —
    see the "Server" section below.
  - TABLEBASE_PATH / TABLEBASE_PATH_7_8 / TABLEBASE_PATH_CASTLING /
    GITHUB_URL — read from the environment with a literal fallback,
    to allow a deployment (for example, a Render Web Service) to select a
    different tablebase location or repository URL without modifying the
    source file; editing the literal value below remains supported.
...
#     of frequently probed materials backed by a remote URL for the rest:
...
# name (falling back to the literal value below when unset). On Render,
# the setting can be supplied from the service configuration without
# modifying the source file. A tablebase set this large (the full ChessTB set runs
...
# (commonly 10000, but not guaranteed) rather than a fixed value. The
# container reads and binds to the injected port. Running locally with no
# PORT set uses 7860.
...
# Worker threads in the probe thread pool for evaluating a position's
...
# (or set to 1 to always parallelize) for workloads with many legal moves
# or when profiling shows sequential probing is a bottleneck. Raise it
# when thread hand-off overhead outweighs the benefit for the workload.
PROBE_PARALLEL_THRESHOLD = 4

# Wall-clock timeout, in seconds, for each probe phase. /probe gives the
# root probe and the child-move batch independent budgets, so a worst-case
# request can consume up to two PROBE_TIMEOUT_SECS intervals. /probe/stream
# uses the same setting for its root probe and child pre-warm/evaluation
# phases. Remaining probes are treated as unknown when a phase reaches its
# deadline rather than blocking the request indefinitely.
...
#                 new enough to expose the table-source seam. Older
#                 versions log the condition and use "download" instead.
...
#
# "direct" is the preferred mode for browsing across many materials.
# "download" is preferable when a small set of materials is accessed
# repeatedly for a long session or when per-request network latency is high.
#
...
# ephemeral local storage) large enough for the materials probed during a
# session. Storage requirements therefore grow with the number and size of
# materials accessed.
REMOTE_MODE = "direct"
...
# Per-client-IP limit applied only to requests that identify themselves as
# background hover-prefetch via the "X-Prefetch: 1" request header (see
# static/js/tablebase.js's hover-prefetch). Deliberately separate from,
# and tighter than, PROBE_RATE_LIMIT -- a client-supplied header is not
# trusted enough to *exempt* a request from limiting altogether (any
# client could tag every request "prefetch" to bypass PROBE_RATE_LIMIT),
# only to route it to its own, smaller budget, so a sustained hover sweep
# can burn through this budget and start getting silently-dropped 429s on
# prefetches without ever touching the user's own click budget above.
# Left empty, prefetch-tagged requests fall back to sharing
# PROBE_RATE_LIMIT with everything else, as if this setting didn't exist.
PREFETCH_RATE_LIMIT = "30 per minute"
...
# redundant under waitress, where waitress performs the same correction
# itself.
#
...
# service exactly once. Increase it when an additional reverse proxy is
# present in front of the service. Set to 0 when no reverse proxy is present
# -- a value that does not match the actual proxy chain
...
TRUSTED_PROXY_COUNT = os.environ.get("TRUSTED_PROXY_COUNT", "1")
...
# behavior) while still being served over plain HTTP. Leaving this True
...
# came back 401.
#
# Read from the environment, like TRUSTED_PROXY_COUNT above, so a
# deployment (e.g. render.yaml's Render blueprint, which sets this to
# "true" explicitly) can configure it without editing this file. Defaults
# to False (via SESSION_COOKIE_SECURE unset or anything other than
# "true") because that's what this project's own documented Quick Start
# needs: `python app.py` served over http://127.0.0.1:7860 -- plain
# HTTP -- after editing only TABLEBASE_PATH above, with DEBUG and this
# setting both left at their defaults.
#
# Set this to "true" for any deployment the browser reaches over HTTPS,
# including one behind a TLS-terminating reverse proxy such as Render's
# (the browser's own connection is what the Secure attribute governs, not
# the internal hop between the proxy and this process).
