...
`static/css/*.css`, `templates/*.html`, and `remote/`,
`Dockerfile`, `render.yaml`, `.dockerignore`, `requirements.txt`, and
`openapi.yaml` — is original and licensed under the terms in
...
- **chessops** (`GPL-3.0-or-later`) — bundled in
  `static/vendor/chessops/` and served to every client.
...
`static/vendor/chessops/` contains the pinned ESM distribution and its
`@badrap/result` runtime dependency. These files are served directly by the
application and are not fetched from a CDN at request time.
