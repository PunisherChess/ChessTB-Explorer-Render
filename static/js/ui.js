 ...
 *     within the existing move tree via Board.goToNode(), which reuses the
 *     already-cached result. Playing a new move after navigating back adds
 *     a variation without removing the existing continuation.
 *   - The settings panel: the cog button in the header opens a dropdown
 *     with three standalone rows: a "Show Root Row" toggle switch (state
 *     owned by tablebase.js; see Tablebase.setShowRootRow/getShowRootRow),
 *     a "Chess960 Mode" toggle switch (state owned by board.js; see
 *     Board.setVariant/getVariant/isChess960), and an "Autoplay Delay"
 *     slider (state owned by this module; see
...
const UI = (() => {
    ...
    // ── URL sharing ───────────────────────────────────────────────────────────
    // variant is appended only for Chess960 — an omitted variant param means
    // Standard (see readHashVariant()), so a Standard-mode link looks
    // exactly as it always has.
    function _writeHash(fen, variant) {
        try {
            let hash = '#fen=' + encodeURIComponent(fen);
            if (variant === VARIANTS.CHESS960) hash += '&variant=chess960';
    ...
    // Returns VARIANTS.CHESS960 / VARIANTS.STANDARD, or null if the hash has
    // no (or an unrecognized) variant param — callers treat null as
    // Standard. Never inferred from the FEN itself (e.g. its castling
    // field) — only this explicit param establishes Chess960 from a URL.
    function readHashVariant() {
        try {
            ...
            return null;   // unknown value — normalize to "unspecified" (Standard)
        } catch (_) { return null; }
    }
    ...
    function readLastGame() {
        try {
            ...
            // Missing or unrecognized variant means Standard.
    ...
    function _buildPgnText() {
        ...
        // Chess960 identity is application metadata a reader must not guess
        // from piece placement — chessops's own PGN Variant parsing maps
        // Chess960 names to the same generic rules Standard uses (see
        // board.js's module doc comment), so this tag is what lets a PGN
        // this app re-imports (or another Chess960-aware reader) recover
        // the ruleset. FEN/SetUp are always included, matching the
        // application's existing PGN format.
    ...
    function onPositionChange(fen, _isLegalMove, immediate) {
        ...
        _updateNavBtns();
        _writeHash(fen, Board.getVariant());
        ...
        const _count = Board.legalMoveCount();
        const _countEl = document.getElementById('move-count');
        if (_countEl) _countEl.textContent = _count > 0 ? `${_count} legal move${_count === 1 ? '' : 's'}` : '';

        const onProbeError = (msg, alreadyRendered) => {
            // alreadyRendered means Tablebase already wrote a more specific
            // message (e.g. the coverage error's piece-count hint) — don't
            // clobber it with the raw server text.
            if (errorEl && !alreadyRendered) errorEl.textContent = msg;
            _setInputsReady(true);   // don't leave inputs stuck disabled on error
        };
        const variant = Board.getVariant();
        if (immediate) {
            // Drop any pending trailing debounced probe — left alone, it
            // would still fire later for the position it was scheduled
            // against and overwrite the results this immediate probe is
            // about to render.
            _debouncedProbe.cancel();
            Tablebase.probe(fen, variant, onProbeError);
        } else {
            _debouncedProbe(fen, variant, onProbeError);
        }
    }
    ...
    // whenever a wait starts, and whenever auto-play stops, so callbacks
    // from older waits can recognise themselves as stale and no-op instead
    // of acting on a position that is no longer active.
    ...
    function _armAutoPlayWait(dataAlreadyReady) {
        ...
        _autoPlayTimer = setTimeout(() => {
            if (cycleId !== _autoPlayCycleId) return;   // inactive wait
    ...
    // result handler below — whichever fires second is the one that moves.
    // cycleId ties the callback to its wait, so callbacks from inactive
    // waits are ignored rather than acting on the wrong position.
    ...
    let _pgnImportedVariant = VARIANTS.STANDARD;      // variant read from the parsed PGN's Variant header
    ...
    // Case-insensitive lookup — a PGN's tags are conventionally capitalized
    // ("FEN", "Variant") but chessops's parser stores whatever case the
    // source PGN actually used, so the key can't be assumed here.
    function _pgnHeaderValue(headers, name) {
    ...
    // chessops's own parseVariant() (used internally by its PGN support)
    // maps every Chess960-like Variant name to the same generic 'chess'
    // rules Standard uses — see board.js's module doc comment — so Chess960
    // identity has to be read from the raw header text here instead of
    // from anything chessops derives from it.
    function _isChess960VariantHeader(value) {
    ...
    function _parsePgn() {
        ...
        let games = [];
        try {
            games = parsePgnStrict(text);
        } catch (error) {
            if (errEl) {
                errEl.textContent = (error instanceof PgnError && error.message === 'ERR_PGN_MULTIPLE_GAMES')
        ...
        _pgnImportedVariant = _isChess960VariantHeader(_pgnHeaderValue(game.headers, 'Variant'))
        ...
        // and moves are only legal relative to that FEN.
        const fenHeader = _pgnHeaderValue(game.headers, 'FEN');
        if (_pgnImportedVariant === VARIANTS.CHESS960 && !fenHeader) {
            if (errEl) errEl.textContent = 'Chess960 PGN requires a FEN starting position.';
            return;
        }
        let pos = null;
        if (fenHeader) {
        ...
        for (const node of moves) {
            ...
            if (!move) {
                if (errEl) errEl.textContent = `Invalid move in PGN: ${node.san}`;
                return;
            }
            pos.play(move);
    ...
    function _renderPgnMoveList() {
        ...
        _pgnSnapshots.forEach((snap, i) => {
            ...
            btn.addEventListener('click', () => {
                ...
                // the current move line, so setPositionAndVariant() (which
                // resets the move line) is correct here — unlike the
                // in-panel move-list click handler above, and unlike
                // setVariant()+setPosition(), which would validate this
                // FEN against the *current* variant on its way to the new
                // one instead of against the variant it's actually meant
                // for. Invokes onPositionChange() itself, so no manual
                // call is needed here.
                const norm   = normalizeFen(snap.fen);
                const result = Board.setPositionAndVariant(norm, _pgnImportedVariant);
                if (!result.ok) {
                    toast(result.reason || 'Could not load that position.');
                    return;
                }
                _closePgnDialog();
            });
    ...
    function _syncPgnRankWidth() {
        const rankHeader = document.querySelector('.moves-table--head th.col-rank');
    ...
    function _initPgnRankWidthSync() {
        const table = document.querySelector('.moves-table--head');
        if (!table) return;
        _syncPgnRankWidth();
    ...
    // .moves-head-scroll (fixed header) and .moves-body-scroll (the actual
    // vertically-scrolling element) are separate elements (see main.css) so
    // the header never shows a scrollbar of its own. When the body's native
    // scrollbar is showing, measure its actual layout width and widen the
    // body table by that amount. This preserves the header/body column
    // alignment without reserving any scrollbar gutter inside the header.
    function _syncMovesGutter() {
        const bodyScroll = document.querySelector('.moves-body-scroll');
        const bodyTable  = bodyScroll?.querySelector('.moves-table--body');
        if (!bodyScroll || !bodyTable) return;

        const overflowing = bodyScroll.scrollHeight > bodyScroll.clientHeight + 1;
        if (!overflowing) {
            bodyScroll.classList.remove('has-vscroll');
            bodyScroll.style.removeProperty('--moves-vscroll-width');
            return;
        }

        const scrollbarWidth = Math.max(0, bodyScroll.offsetWidth - bodyScroll.clientWidth);
        bodyScroll.style.setProperty('--moves-vscroll-width', `${scrollbarWidth}px`);
        bodyScroll.classList.add('has-vscroll');
    }

    function _initMovesGutterSync() {
        const bodyScroll = document.querySelector('.moves-body-scroll');
        const tbody      = document.getElementById('moves-tbody');
        if (!bodyScroll || !tbody) return;
        _syncMovesGutter();
        // Rows changing height/count (tbody) and the panel resizing
        // (bodyScroll) are the two things that can toggle whether the
        // body actually needs to scroll.
        new ResizeObserver(_syncMovesGutter).observe(bodyScroll);
        new ResizeObserver(_syncMovesGutter).observe(tbody);
        window.addEventListener('resize', _syncMovesGutter, { passive: true });
    }
    ...
    // ── Settings panel ────────────────────────────────────────────────────────
    //
    // Dropdown anchored under the header cog button, holding three
    // standalone rows: the Show Root Row toggle (state owned by
    // tablebase.js), the Chess960 Mode toggle (state owned by board.js —
    // see Board.setVariant/getVariant/isChess960), and the Autoplay Delay
    // slider (state owned by this module — see _AUTO_PLAY_DELAY_DEFAULT_MS
    // and _autoPlayDelayMs in the Auto-play section above). The board and
    // piece set are fixed (theme.js) and have no picker here.
    ...
    function _initSettings() {
        ...
        cogBtn.addEventListener('click', e => {
            ...
            if (opening) {
                ...
                const chess960Toggle = document.getElementById('chess960-mode-toggle');
                if (chess960Toggle) chess960Toggle.checked = Board.isChess960();
        ...
        // Chess960 Mode toggle. Board.setVariant() stops auto-play, resets
        // the move tree, and re-probes under the new variant via its own
        // onPositionChange() call when the switch succeeds (see board.js).
        // It can also refuse the switch outright — e.g. the current
        // position's castling rights only make sense under Chess960 rules —
        // in which case the position is left exactly as it was and the
        // switch itself must be reverted here too, or the toggle would show
        // a mode the board isn't actually in.
        const chess960Toggle = document.getElementById('chess960-mode-toggle');
        if (chess960Toggle) {
            chess960Toggle.checked = Board.isChess960();
            chess960Toggle.addEventListener('change', () => {
                const wantChess960 = chess960Toggle.checked;
                const result = Board.setVariant(wantChess960 ? VARIANTS.CHESS960 : VARIANTS.STANDARD);
                if (!result.ok) {
                    chess960Toggle.checked = !wantChess960;
                    toast(result.reason || "Can't switch modes for this position.", 3000);
                }
            });
        }
    ...
    function _setActiveMetric(metric) {
        // Both the header and body tables carry their own copy of the
        // metric column group (see index.html), so both need the attribute
        // for the [data-active-metric] rules in main.css to hide/show the
        // right columns consistently across the two.
        document.querySelectorAll('.moves-table').forEach(table => {
            table.dataset.activeMetric = metric;
        });
    ...
    function _bindKeys() {
        document.addEventListener('keydown', e => {
            ...
            // Same reasoning for the PGN import dialog and settings panel —
            // a focused button inside either still lets keydown reach here,
            // so shortcuts must not act on the board underneath them.
            if (document.getElementById('pgn-dialog')?.classList.contains('is-open')) return;
            ...
            // Board.flip() clears the (orientation-dependent) best-move
            // arrows and expects the caller to redraw them — mirror the
            // flip-btn click handler here so the keyboard path doesn't
            // leave them missing.
    ...
    // readLastGame()): {variant, startFen, nodes, rootChildren,
    // rootActiveChild, currentId}. `initialFen` in that case is expected to
    // already equal restoreGame.startFen — Board.init() below loads it as
    // the tree's starting position, then restoreTree() fast-forwards
    // straight to the saved node (rebuilding the PGN panel, variations
    // included) without re-probing every intermediate move.
    //
    // `initialVariant`, if given, is established before initialFen is
    // parsed into the rules engine (see board.js's module doc comment) —
    // app.js resolves it the same way it resolves initialFen, from the URL
    // hash first and the saved game snapshot otherwise.
    function init(initialFen, restoreGame, initialVariant) {
    ...
    return { init, normalizeFen, readHashFen, readHashVariant, readLastGame, toast };
