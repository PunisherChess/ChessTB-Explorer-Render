 ...
 *     probed position (e.g. clicking back into the move line, or the PGN
 *     panel) reuses the cached result instead of issuing a new
 *     /probe/stream call — keyed by variant as well as FEN because the
 *     same FEN string probes to a different result under Standard vs
 *     Chess960 rules (see config.py's Chess960 tablebase routing).
 *   - New probes blank the table immediately so only the is-loading shimmer
 *     remains visible until the new probe resolves.
 */
...
const Tablebase = (() => {
    ...
    // A FEN string alone is not a stable cache/identity key — the same FEN
    // probes to a different (and possibly incompatible) result under
    // Standard vs Chess960 rules, so every cache below is keyed by this
    // composite rather than by fen alone.
    const VARIANT_STANDARD = 'standard';
    function _cacheKey(fen, variant) { return `${variant || VARIANT_STANDARD}\u0000${fen}`; }
    ...
    //
    // `interactive` cells live in #moves-tbody, a separate <table> from the
    // column headers (see index.html), so the browser's automatic header
    // association doesn't apply — `headers` points each cell at its group
    // and column <th> ids explicitly. Root-row cells stay in the header
    // table alongside those <th>s and don't need it.
    function _createMoveCells(entry, groupClass, withLabel, interactive = true, dtcFormat = false) {
        ...
        if (!entry) {
            ...
            es.textContent = '—';
            if (headers) { em.setAttribute('headers', headers.move); es.setAttribute('headers', headers.score); }
            return [em, es];
        }
        const moveCell = document.createElement('td');
        moveCell.className = `${groupClass} col-move${interactive ? '' : ' is-root'}`;
        if (headers) moveCell.setAttribute('headers', headers.move);
        if (interactive) {
        ...
        if (headers) scoreCell.setAttribute('headers', headers.score);
    ...
    function _renderTable(movesDtz, movesDtc, movesDtm, movesDtm50) {
        ...
        for (let i = 0; i < maxRows; i++) {
            ...
            rankCell.setAttribute('headers', 'th-rank');
    ...
    // Clears the move table to its blank loading state. Used whenever a
    // new network probe is started; cache and prefetch hits resolve without
    // a blank interstitial.
    //
    ...
    function _applyCachedResult(data, fen, variant) {
        _setLoading();
    ...
    function _handleError(errData, errorEl, onError) {
        ...
        // isCoverageError tells the caller a friendlier message (with the
        // piece-count hint) is already rendered, so it doesn't overwrite it
        // with this raw server message.
    ...
    // ── Main probe function ───────────────────────────────────────────────────
    // variant must be one of board.js's VARIANTS values ('standard' /
    // 'chess960') — callers (ui.js) read it from Board.getVariant() at call
    // time, so a debounced call always carries whichever variant was active
    // when it was scheduled, not whichever is active when it eventually
    // fires (utils.js's debounce() re-captures the latest call's arguments,
    // including this one).
    function probe(fen, variant, onError) {
        ...
        if (_prefetchCache.has(key)) {
            ...
            _applyCachedResult(cached, fen, variant);
            return;
        }
        ...
        if (_resultCache.has(key)) {
            _applyCachedResult(_resultCache.get(key), fen, variant);
            return;
        }
        ...
        // Blank the table/WDL while this probe is in flight so only the
        // loading shimmer remains visible.
        _blankResults();
        _setLoading();
        _showProgress(0, 0);
        const errorEl = document.getElementById('error-line');
    ...
    function exportCsv() {
        ...
        if (_showRootRow) {
            const dtcRootRaw = _dtcRootRaw(_lastData);
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        tbody.addEventListener('mouseover', e => {
            ...
            const variant = Board.getVariant() || VARIANT_STANDARD;
            const key = _cacheKey(childFen, variant);
            if (key === _prefetchKey) return;   // already pending/in-flight for this move
            _cancelPrefetch();                  // hover moved elsewhere — drop the old one
            if (_prefetchCache.has(key) || _resultCache.has(key)) return;
            _prefetchKey = key;
            // A short dwell fires a prefetch for cells the pointer pauses on.
            _prefetchTimer = setTimeout(() => {
                ...
                .finally(() => {
                    ...
                    if (_prefetchKey === key) _prefetchKey = null;
                });
            }, 400);
        });
    ...
    return { probe, setMoveSelectHandler, setResultHandler, getLastData, getLastFen, getLastVariant, setShowRootRow, getShowRootRow };
