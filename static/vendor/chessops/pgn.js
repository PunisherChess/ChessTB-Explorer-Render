...
export class PgnParser {
    constructor(emitGame, initHeaders = defaultHeaders, maxBudget = 1000000, strict = false) {
        ...
        continuedLine: for (;;) {
            switch (this.state) {
                ...
                case 3 /* ParserState.Moves */: {
                    ...
                    const tokenRegex = /(?:[NBKRQ]?[a-h]?[1-8]?[-x]?[a-h][1-8](?:=?[nbrqkNBRQK])?|[pnbrqkPNBRQK]?@[a-h][1-8]|[O0o][-–—][O0o](?:[-–—][O0o])?)[+#]?|--|Z0|0000|@@@@|{|;|\$\d{1,4}|[?!]{1,2}|\(|\)|\*|1[-–—]0|0[-–—]1|1\/2[-–—]1\/2|\d+\.(?:\.\.)?/g;
                    let match;
                    let tokenEnd = 0;
                    while ((match = tokenRegex.exec(line))) {
                        if (this.strict && /\S/.test(line.slice(tokenEnd, match.index)))
                            throw new PgnError('ERR_PGN_TOKEN');
                        tokenEnd = tokenRegex.lastIndex;
                        const frame = this.stack[this.stack.length - 1];
                        let token = match[0];
                        if (this.strict && this.resultSeen)
                            throw new PgnError('ERR_PGN_AFTER_RESULT');
                        if (token === ';')
                            return;
                            ...
                            if (this.strict) {
                                if (this.stack.length !== 1)
                                    throw new PgnError('ERR_PGN_RESULT');
                            ...
                            if (this.stack.length > 1)
                                ...
                                throw new PgnError('ERR_PGN_VARIATION');
                        }
                        else if (token === '{') {
                    ...
                    if (this.strict && /\S/.test(line.slice(tokenEnd)))
                        throw new PgnError('ERR_PGN_TOKEN');
                    if (this.strict && this.stack.length !== 1)
                        throw new PgnError('ERR_PGN_VARIATION');
                    return;
                }
    ...
    emit(err) {
        ...
        if (this.strict && this.state === 4 /* ParserState.Comment */)
            throw new PgnError('ERR_PGN_COMMENT');
...
export const parsePgnStrict = (pgn, initHeaders = defaultHeaders) => {
    const games = [];
    let error;
    new PgnParser((game, err) => {
        if (err) error = err;
        games.push(game);
    }, initHeaders, NaN, true).parse(pgn);
    if (error) throw error;
    if (games.length > 1) throw new PgnError('ERR_PGN_MULTIPLE_GAMES');
    return games;
};
