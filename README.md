...
- **Auto-play** — automatically plays the best move for any of the four
  metrics on a timer, allowing a line to play out automatically. The per-move delay
...
Browser (vanilla JS, ES modules)
  ├─ board.js      — chessops-backed position/rules state, chessground
...
4. **Configure `config.py`** — open it and set `TABLEBASE_PATH` to the
   ...
   list. Open the file and edit the values directly. To use
...
5. **Run the app:**
   ...
   **waitress**, a production-grade pure-Python WSGI server — see
   ...
   Set `DEBUG = True` in `config.py` to use Flask's development server
...
`TABLEBASE_PATH*` settings at the appropriate part of that layout
...
`REMOTE_MODE` selects the remote access mode. This is a fixed, explicit choice configured via
config, not something the app detects or switches automatically at
runtime; the configured value remains active for the life of the process:
...
- **`"direct"` (default)** — `remote/remote_direct.py` probes the remote
  ...
  fork (see `requirements.txt`) always has.
...
is what this app does. `"download"` is preferable when a small set of
...
This mode is suitable for a machine that does not have sufficient local
disk to dedicate a couple of terabytes of local disk for the full tablebase set —
at the cost of probe latency on first touch of any given material
(depending on the connection to the CDN), some temporary local disk
usage for the materials probed during the session, and of
...
|-----------------------------|-------------------|-------------|
| `TABLEBASE_PATH`            | `""` (also readable from a `TABLEBASE_PATH` env var) | Directory containing the ChessTB tablebase files, **or** an `http(s)://` base URL serving the same layout remotely (e.g. a Hugging Face storage bucket — see [Getting the tablebase files](#getting-the-tablebase-files)). Required for probing to work. On Render, set this from the service's Environment tab (or `render.yaml`'s prompt) instead of editing the file. |
...
| `PREFETCH_RATE_LIMIT`       | `"30 per minute"` | Per-client-IP request limit for hover prefetch requests marked with `X-Prefetch: 1`. Left empty, prefetch rate limiting is disabled. See [Security notes](#security-notes). |
...
| `SESSION_COOKIE_SECURE`      | `False` (also readable from a `SESSION_COOKIE_SECURE` env var, `"true"`/anything else) | Whether the admin session cookie set by `/admin/login` requires HTTPS. Independent of `DEBUG`. Defaults `False` to match this project's own unmodified, plain-HTTP local Quick Start; set to `"true"` for any deployment the browser reaches over HTTPS (including behind a TLS-terminating reverse proxy such as Render's — `render.yaml` already sets this explicitly for that deployment). See [Security notes](#security-notes). |
...
mirroring step or GitHub Action required. Render watches the connected
branch and rebuilds/redeploys automatically on every push, the same way
...
4. Under **Environment**, add:
   ...
   - `ADMIN_TOKEN` — optional but recommended; any secret string, or
     use Render's "Generate" button. Leaving it unset causes
     `/admin` responds `503` while it remains unset.
   - `FLASK_SECRET_KEY` — optional; a random one is generated at
     process startup when it is omitted; admin sessions
   ...
   - `SESSION_COOKIE_SECURE` — set to `true`. Render always terminates
     ...
     proxy; `config.py`'s own default (`false`) is tuned for the
     unmodified local Quick Start instead, not this deployment. (Option
     1's `render.yaml` sets this automatically — it is only a
     manual step here because Option 2 skips that file.)
   - `GITHUB_URL` — optional; the repository URL, when the header's
...
works for casual or low-traffic use; the service spins
...
`variant` is optional and defaults to `"standard"` when omitted, for
compatibility with clients sent before Chess960 support existed. It is
...
├── tablebase_opening.py    # Per-material table-opening lock policy, shared by tablebase_router.py and remote/remote_direct.py
├── remote/
...
└── static/
    ...
    ├── js/
    │   ├── app.js           # Bootstrap
    │   ├── board.js         # Chessops-backed position/rules state + chessground wrapper (drag/drop, history, arrows, variant)
    ...
    └── vendor/               # Third-party libraries (Chessground, chessops, fonts)
        └── chessops/          # Vendored application dependency; not fetched from a CDN at request time
```
...
- **`SESSION_COOKIE_SECURE`** (`config.py`'s "Admin" section) controls
  ...
  `DEBUG = False` alone doesn't imply HTTPS is actually in front of this
  ...
  unmodified, plain-HTTP local Quick Start — a browser won't send a
